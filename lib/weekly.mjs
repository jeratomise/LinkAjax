...
id: ${id}
status: pending
pillar: ${post.pillar || "marketing-os"}
theme: ${post.theme}
source_links: []
voice: provisional
...
`;
    ...
    written.push(file);
    indexPosts.push({ id, file, status: "pending", pillar: post.pillar || "marketing-os", theme: post.theme });
  });

  writeUtf8(
    path.join(queueDir, "index.json"),
    JSON.stringify({ weekOf: iso, status: "pending-approval", theme: posts[0]?.theme || "", posts: indexPosts }, null, 2) + "\n",
  );
